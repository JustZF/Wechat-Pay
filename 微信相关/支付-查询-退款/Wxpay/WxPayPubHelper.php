	...
	function createXml()
	{
		try
		{
			...
			if($this->parameters["out_trade_no"] == null && $this->parameters["transaction_id"] == null) {
				...
				throw new SDKRuntimeException("退款申请接口中，缺少必填参数out_refund_no！"."<br>");//随机数
			}elseif($this->parameters["total_fee"] == null){
			...
			// 	throw new SDKRuntimeException("退款申请接口中，缺少必填参数op_user_id！"."<br>");//商户号
			// }
		   	$this->parameters["appid"] = WxPayConf_pub::APPID;//公众账号ID
		   	$this->parameters["mch_id"] = WxPayConf_pub::MCHID;//商户号
		    $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
...
class NativeLink_pub  extends Common_util_pub
{
	...
	function createLink()
	{
		...
		{		
			...
			{
				throw new SDKRuntimeException("null product_id！"."<br>");
			}			
		   	$this->parameters["appid"] = WxPayConf_pub::APPID;//公众账号ID
		   	$this->parameters["mch_id"] = WxPayConf_pub::MCHID;//商户号
		   	$time_stamp = time();
